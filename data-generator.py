...
def pattern_designer():
    ...
    t = np.where(t == 0, 0.1, 0.9)
    t_degisik = np.copy(t)
    t_degisik[0 ,0],t_degisik[1 ,0],t_degisik[3 ,4],t_degisik[3 ,5] = 0.9,0.1,0.1,0.9
    l= np.ones((5, 10))
    l[:, 0] = 0
    l[-1, :] = 0
    l = np.where(l == 0, 0.1, 0.9)
    l_degisik = np.copy(l)
    l_degisik[4 ,4],l_degisik[3, 4],l_degisik[4, 9],l_degisik[3 ,9] = 0.9,0.1,0.9,0.1
    h= np.ones((5, 10))
    h[:, 2] = 0
    h[:, 6] = 0
    h[2, :] = 0
    ...
    h_degisik = np.copy(h)
    h_degisik[0,8],h_degisik[2,6],h_degisik[0,0],h_degisik[2,2] = 0.1,0.9,0.1,0.9
    a= np.ones((5, 10))
    a[:, 9] = 0
    a[0, :] = 0
    a = np.where(a == 0, 0.1, 0.9)
    a_degisik = np.copy(a)
    a_degisik[0, 2],a_degisik[1, 2],a_degisik[3, 8],a_degisik[3, 9] = 0.9,0.1,0.1,0.9
...
x_input = [t, h, a, l, t_degisik, h_degisik, a_degisik, l_degisik, t_noisy, h_noisy, a_noisy, l_noisy]
for i in x_input:
...
x_test = data_vectorizer([t, h, a, l, t_noisy_test, h_noisy_test, a_noisy_test, l_noisy_test])
y_desired_test = [e1, e2, e3, e4, e1, e2, e3, e4]

#Veriler tekrar kullanılmak için kaydedildi
np.save("x_egitim.csv", x_input)
np.save("yd_egitim.csv", y_desired)
np.save("x_test.csv", x_test)
np.save("yd_test.csv", y_desired_test)
